/*
 *  hp-wireless button repurposed
 *
 *  Copyright (C) 2014 Alex Hung <alex.hung@canonical.com>
 *  Copyright (C) 2016 Andrew Chen <andrew.chuanye.chen@gmail.com>
 *
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Andrew Chen");
MODULE_ALIAS("acpi*:HPQ6001:*");

// Generate KEY_HP on press by default
static int keycode = KEY_HP;
module_param(keycode, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(keycode, "Keycode to emit when the button is pressed");
...
static void hpwl_notify(struct acpi_device *acpi_dev, u32 event)
{
	...
	input_report_key(hpwl_input_dev, keycode, 1);
	input_sync(hpwl_input_dev);
	input_report_key(hpwl_input_dev, keycode, 0);
	input_sync(hpwl_input_dev);
}
